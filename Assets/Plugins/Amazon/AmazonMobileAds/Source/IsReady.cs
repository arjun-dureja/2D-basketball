...
namespace com.amazon.mas.cpt.ads
{
    public sealed class IsReady : Jsonable
    {
        ...
        {
            Dictionary<string, IsReady> result = new Dictionary<string, IsReady>();
            foreach (var entry in jsonMap)
            {
                IsReady value = CreateFromDictionary(entry.Value as Dictionary<string, object>);
        ...
        {
            List<IsReady> result = new List<IsReady>();
